...
| 2414166-滕鑫 | Codex 桌面应用 | GPT-6 | 用于整理 GDB 调试步骤和寄存器观察结果 |
...
本次还体会到提示词需要包含代码位置、运行环境、期望观察点和验证标准，要尽量详细和清晰。要求 AI 区分伪指令与机器指令、区分内存装载与执行跳转，并要求结论引用实际日志，可以明显减少含糊或想当然的回答。所有使用过的提示词汇总在同目录的 `prompt.md`。
